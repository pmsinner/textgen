...
if namespace.erasepunctuation:
	text = filter(lambda x: not x in commas, text).encode("utf-8")
...
if namespace.generate:
	...
	if namespace.seed == "":
		...
		sentence = namespace.seed
	generation += sentence
	print('----- Starting with: "' + sentence + '"')
	...
	for t in text:
		if not t in schs:
			...
			ntxt = ntxt + " "
	tsp = sorted(list(set(ntxt.split())))
	tsp.append("@")
	tsp.append("\n")
	tps =[]
	for ch in tsp:
		tps.append(ch.decode("utf-8"))
	out = ""
	while len(generation) < namespace.genlength:
		...
		if namespace.wordsonly:
			if next_char == " ":
				gpl = generation.split()[::-1][0]
				if not gpl in tsp:
					a = 2
					if "".join(generation.split(" ")[len(generation.split(" ")) - 2].split()).decode("utf-8", "ignore") == "":
						a+=1
					gr = generation.split()[::-1]

					del gr[0]
					generation = " ".join(gr[::-1])
				else:	
					out = out + " " + generation.split()[::-1][0]
			clearConsole()
			print(out)
		else:
...
for iteration in range(1, namespace.epoch + 1):
	...
	while len(generated) < 200:
		...
		generated += next_char
		#if next_char == " ":
			#if not generated.split(" ")[len(generated.split(" ")) - 1] in text.split(" "):
				#generated = " ".join(generated.split(" ")[0:len(generated.split(" ")) - 2])
		sentence = sentence[1:] + next_char
		sys.stdout.write(next_char)
		sys.stdout.flush()
	print()

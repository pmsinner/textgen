...
for i in range(namespace.layers - 1): #Добавляем рекуррентные LSTM
	model.add(GRU(dimensions, return_sequences=True))
	model.add(Dropout(0.07))
model.add(GRU(dimensions, return_sequences=False))
...
for iteration in range(1, namespace.epoch + 1):
	...
	history = History()
	history = model.fit(X, y, batch_size=namespace.batchsize, nb_epoch=1) #Прогоняем текстовые данные по одной эпохе, после каждой пишем пробный текст
	xs.append(iteration)
	ys.append(history.history['loss'][0])
	f = open('pltGRU', "w")
	f.write(" ".join(xs) + "\n" + " ".join(ys))
	f.close()
